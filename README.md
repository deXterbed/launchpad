...
This extension is published on [Open VSX](https://open-vsx.org) only (used by Cursor, VSCodium, and other editors).

### 1. Prerequisites

- **Open VSX account** – Sign in at [open-vsx.org](https://open-vsx.org) (e.g. with GitHub).
- **Publisher agreement** – Accept the [Publisher Agreement](https://open-vsx.org/user-settings/namespace-access) on Open VSX.
- **package.json** – Set `"publisher": "your-namespace"` in `package.json` to the namespace you’ll use on Open VSX (e.g. your GitHub username).

### 2. Create namespace (first time only)

Create a namespace that matches your `publisher` id:

```bash
npx ovsx create-namespace your-namespace
```

Sign in when prompted (or use a token). Use the same value as `publisher` in `package.json`.

### 3. Access token

1. Go to [Open VSX: Access tokens](https://open-vsx.org/user-settings/tokens).
2. Click **Generate new token**, give it a name, then copy the token (it’s shown only once).

### 4. Publish

```bash
npm run package              # build the extension
npx ovsx publish -p <token>  # publish to Open VSX
```

Or in one step (ovsx runs the prepublish script):

```bash
npx ovsx publish -p <token>
```

- **Bump version:** edit `version` in `package.json` before publishing.
- **Manage:** [Open VSX publisher dashboard](https://open-vsx.org/user-settings/extensions) to unpublish or update.
